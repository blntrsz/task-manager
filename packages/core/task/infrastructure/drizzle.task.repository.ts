...
import { TaskEntity, type TaskEntitySchema } from "../domain/task.entity.js";
import type { TaskRepository } from "../domain/task.repository.js";
import { taskTable } from "./task.sql.js";

export class DrizzleTaskRepository implements TaskRepository {
	constructor(private readonly db: PgliteDatabase<Record<string, never>>) {}
	...
	async findAllPaginated({
		limit,
		page,
	}: { page: number; limit: number }): Promise<Paginated<TaskEntity>> {
	...
	async insert(entity: TaskEntity): Promise<TaskEntity> {
		const [createdEntity] = await this.db
			.insert(taskTable)
			.values(entity.toPlain())
			.returning()
			.execute();
